...
/* Создать контекст заранее, до первого звука.

   Это не микрооптимизация. Создание AudioContext синхронное и
   тяжёлое: оно поднимает звуковой поток. Раньше оно случалось в
   момент первого звука, то есть ровно в кадр, когда ERIPOV касается
   строки, и роняло этот кадр — падение слова видимо подвисало.
   Теперь контекст готовится в паузе перед выкатом, когда на экране
   ничего не движется и потерять кадр не жалко. */
